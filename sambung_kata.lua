...
local DictionaryURL = "https://raw.githubusercontent.com/geovedi/indonesian-wordlist/master/00-indonesian-wordlist.lst"

local ZeyModz = (function()

if debugX then
	warn('Initialising ZeyModz')
end
...
local ZeyModzFolder = "ZeyModz"
local ConfigurationFolder = ZeyModzFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {

		ZeyModzOpen = {Type = 'bind', Value = 'F1', Name = 'ZeyModz Keybind'},
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("ZeyModz | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, ZeyModzFolder) then
				if callSafely(isfile, ZeyModzFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, ZeyModzFolder..'/settings'..ConfigurationExtension)
				end
			end

			if useStudio then
				file = [[
		{"General":{"ZeyModzOpen":{"Value":"F1","Type":"bind","Name":"ZeyModz Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"ZeyModz Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"F1"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
			...
			if #file > 0 then
				...
				for settingName, settingValue in overriddenSettings do
					...
					assert(#split == 2, "ZeyModz | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('ZeyModz had an issue accessing configuration saving capability.')
		end
	end
end
...
local ZeyModz = useStudio and script.Parent:FindFirstChild('ZeyModz') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local ZeyModzDestroyed = false

repeat
	if ZeyModz:FindFirstChild('Build') and ZeyModz.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('ZeyModz | Build Mismatch')
		print('ZeyModz may encounter issues as you are running an incompatible interface version ('.. ((ZeyModz:FindFirstChild('Build') and ZeyModz.Build.Value) or 'No Build') ..').\n\nThis version of ZeyModz is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, ZeyModz = ZeyModz, useStudio and script.Parent:FindFirstChild('ZeyModz') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

ZeyModz.Enabled = false

if gethui then
	ZeyModz.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(ZeyModz)
	ZeyModz.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	ZeyModz.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ZeyModz.Name and Interface ~= ZeyModz then
			Interface.Enabled = false
			Interface.Name = "ZeyModz-Old"
		end
	end
elseif not useStudio then
...
if ZeyModz.AbsoluteSize.X < minSize.X and ZeyModz.AbsoluteSize.Y < minSize.Y then
...
local Main = ZeyModz.Main
local MPrompt = ZeyModz:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = ZeyModz:FindFirstChild('Drag')
...
ZeyModz.DisplayOrder = 100
LoadingFrame.Version.Text = Release

local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/ZeyModz/refs/heads/main/icons.lua')
...
local Notifications = ZeyModz.Notifications
local keybindConnections = {}

local SelectedTheme = ZeyModzLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = ZeyModzLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	ZeyModz.Main.BackgroundColor3 = SelectedTheme.Background
	ZeyModz.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ZeyModz.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ZeyModz.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	ZeyModz.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ZeyModz.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ZeyModz.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		ZeyModz.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		ZeyModz.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(ZeyModz:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("ZeyModz | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("ZeyModz | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('ZeyModz had an issue decoding the configuration file, please try delete the file and reopen ZeyModz.') return end

	for FlagName, Flag in pairs(ZeyModzLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("ZeyModz | Unable to find '"..FlagName.. "' in the save file.")
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(ZeyModzLibrary.Flags) do
...
function ZeyModzLibrary:Notify(data)
	...
	task.spawn(function()
		...
		if data.Actions then
			warn('ZeyModz | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			ZeyModzLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			ZeyModzLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "ZeyModzOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings()
	...
	if success then
		...
		callSafely(writefile, ZeyModzFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('ZeyModz Settings', 0, true)

	if TabList['ZeyModz Settings'] then
		TabList['ZeyModz Settings'].LayoutOrder = 1000
	end

	if Elements['ZeyModz Settings'] then
		Elements['ZeyModz Settings'].LayoutOrder = 1000
	end
...
function ZeyModzLibrary:CreateWindow(Settings)
	if ZeyModz:FindFirstChild('Loading') then
		if getgenv and not getgenv().ZeyModzCached then
			ZeyModz.Enabled = true
			ZeyModz.Loading.Visible = true

			task.wait(1.4)
			ZeyModz.Loading.Visible = false
		end
	end

	if getgenv then getgenv().ZeyModzCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				ZeyModzLibrary:Notify({Title = 'Build Mismatch', Content = 'ZeyModz may encounter issues as you are running an incompatible interface version ('.. ((ZeyModz:FindFirstChild('Build') and ZeyModz.Build.Value) or 'No Build') ..').\n\nThis version of ZeyModz is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ZeyModz"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "ZeyModz Interface Suite" then
		LoadingFrame.Version.Text = "ZeyModz UI"
	end
	...
	local function CleanUI(obj)
		if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
			obj.Text = obj.Text:gsub("Rayfield", "ZeyModz")
		end
		if obj.Name:find("Rayfield") then
			obj.Name = obj.Name:gsub("Rayfield", "ZeyModz")
		end
		if obj:IsA("ImageLabel") and (obj.Image:find("7300477598") or obj.Image:find("7300480952")) then
			obj.Visible = false
		end
	...
	if not Settings.DisableZeyModzPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				ZeyModzLibrary:Notify({
					Title = "ZeyModz Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(ZeyModzFolder.."/Discord Invites")

		if callSafely(isfile, ZeyModzFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then
				callSafely(writefile, ZeyModzFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ZeyModz RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(ZeyModzFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("ZeyModz | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZeyModz specific development.')
				end
			end
		end
		...
		if callSafely(isfile, ZeyModzFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, ZeyModzFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, ZeyModzFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						ZeyModzLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZeyModz | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZeyModz specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings)
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ZeyModzLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ZeyModzLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			ZeyModz.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ZeyModz | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZeyModz specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					ZeyModzLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("ZeyModz | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with ZeyModz specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ZeyModzLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ZeyModz | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with ZeyModz specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ZeyModzLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= ZeyModzLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZeyModz | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZeyModz specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						ZeyModzLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ZeyModz | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with ZeyModz specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ZeyModzLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			ZeyModzLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			ZeyModzLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('ZeyModz had an issue creating settings.') end
...
function ZeyModzLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function ZeyModzLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection
function ZeyModzLibrary:Destroy()
	ZeyModzDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['ZeyModz Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "ZeyModzOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function ZeyModzLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				ZeyModzLibrary:Notify({Title = "ZeyModz Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			ZeyModzLibrary:Notify({Title = "ZeyModz Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('ZeyModz Configurations Error | '..tostring(result))
			ZeyModzLibrary:Notify({Title = "ZeyModz Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	ZeyModzLibrary.LoadConfiguration()
...
return ZeyModzLibrary

end)()

local Config = {
    Enabled = false,
    ScanKey = Enum.KeyCode.Space,
    Delay = 0.05,
    UsedWords = {},
...
local CharToKey = {
    ...
    ["U"] = Enum.KeyCode.U, ["V"] = Enum.KeyCode.V, ["X"] = Enum.KeyCode.X, ["Y"] = Enum.KeyCode.Y,
    ["Z"] = Enum.KeyCode.Z, ["W"] = Enum.KeyCode.W,
...
local function PressKey(k)
    if not VirtualInputManager or not k then return end
    pcall(function()
...
SelectionUI.Name = "ZeyModzSelection"
SelectionUI.IgnoreGuiInset = true
SelectionUI.Parent = (game:GetService("CoreGui"):FindFirstChild("RobloxGui") or gethui())

local SelectionFrame = Instance.new("Frame")
SelectionFrame.Name = "ScanArea"
...
SelectionFrame.BorderColor3 = Color3.fromRGB(0, 255, 0)
SelectionFrame.Position = UDim2.fromScale(Config.SelectionPos.X, Config.SelectionPos.Y)
SelectionFrame.Size = UDim2.fromScale(Config.SelectionSize.X, Config.SelectionSize.Y)
SelectionFrame.Visible = false
SelectionFrame.Parent = SelectionUI

local selectionLabel = Instance.new("TextLabel")
selectionLabel.Text = "AREA SCAN (SPACE TO TRIGGER)"
selectionLabel.Size = UDim2.fromScale(1, 1)
...
local function LoadDictionary()
    local success, result = pcall(function()
        return game:HttpGet(DictionaryURL)
    end)

    if success then
        local words = string.split(result, "\n")
        local count = 0
        for _, word in ipairs(words) do
            local cleanWord = word:gsub("%s+", ""):upper()
            if #cleanWord > 1 then
                table.insert(WordList, cleanWord)
                count = count + 1
            end
        end
        return count
...
local function FindWord(prefix)
    ...
    for _, word in ipairs(WordList) do
        if word:sub(1, #prefix) == prefix and not Config.UsedWords[word] then
    ...
    if #matches > 0 then
        local randomWord = matches[math.random(1, #matches)]
        return randomWord, #matches
    end
    return nil, 0
end

local function DirectType(fullWord, prefix)
    local suffix = fullWord:sub(#prefix + 1):upper()
    
    ZeyModz:Notify({
        Title = "ZeyModz - Auto Typing",
        Content = "Suffix: " .. suffix .. " | Word: " .. fullWord,
        Duration = 2
    })
...
local function PerformScan()
    if Config.IsScanning then return end
    Config.IsScanning = true
    
    local success, err = pcall(function()
        if not SelectionFrame or not SelectionFrame:IsDescendantOf(game) then
            ZeyModz:Notify({Title = "ZeyModz Error", Content = "Scan Area missing! Please restart the script.", Duration = 4})
            return
        end
        ...
        for _, desc in ipairs(PlayerGui:GetDescendants()) do
            if desc:IsA("TextLabel") and desc.Visible then
                ...
                if desc.Parent:IsA("GuiObject") then parentVisible = desc.Parent.Visible
                elseif desc.Parent:IsA("ScreenGui") then parentVisible = desc.Parent.Enabled end

                if not parentVisible then continue end
                ...
                center.Y >= absPos.Y and center.Y <= (absPos.Y + absSize.Y) then
                    ...
                    local rawText = desc.Text:gsub("%s+", ""):upper()
                    if #rawText >= 1 and #rawText <= 5 and rawText:match("^%a+$") then
                        if not rawText:find("ZEY") and not rawText:find("SKOR") then
                            if desc.TextSize > maxTextSize then
                                maxTextSize = desc.TextSize
                                anchorY = center.Y
                            end
                            table.insert(rawLabels, {
                                Label = desc,
                                PosX = p.X,
                                CenterY = center.Y,
        ...
        local promptLabels = {}
        for _, item in ipairs(rawLabels) do

            if item.TextSize >= (maxTextSize - 2) and math.abs(item.CenterY - anchorY) < 15 then
                table.insert(promptLabels, item)
            end
        end

        table.sort(promptLabels, function(a, b)
            return a.PosX < b.PosX
        end)

        if #promptLabels > 0 then
            local text = ""
            for i = 1, math.min(#promptLabels, 3) do
                text = text .. promptLabels[i].Text
            end

            local match, count = FindWord(text)
            if match then
                ZeyModz:Notify({
                    Title = "ZeyModz - " .. count .. " Words Found",
                    Content = "Prompt: " .. text .. " | Typing: " .. match,
                    Duration = 2
                })
                task.wait(Config.Delay)
                DirectType(match, text)
                Config.UsedWords[match] = true
            else
                ZeyModz:Notify({Title = "ZeyModz", Content = "No words for: " .. text, Duration = 2})
            end
        else
            ZeyModz:Notify({Title = "ZeyModz", Content = "Letters not found!", Duration = 2})
        end
    end)
    
    if not success then
        ZeyModz:Notify({Title = "ZeyModz - Internal Error", Content = tostring(err):sub(1, 100), Duration = 5})
        warn("ZeyModz PerformScan Error: " .. tostring(err))
    end
    
    task.delay(0.5, function() Config.IsScanning = false end)
end
...
UserInputService.InputBegan:Connect(function(input, gpe)
    if input.KeyCode == Config.ScanKey then
        if not Config.Enabled then
            ZeyModz:Notify({
                Title = "ZeyModz - OFF",
                Content = "Script is not active! Toggle ON in the menu first.",
                Duration = 2
            })
            return
        end
...
MainTab:CreateLabel("Menu Keybind : F1")

local count = 0
local loadSuccess, loadErr = pcall(function()
    count = LoadDictionary()
end)

if not loadSuccess then
    warn("ZeyModz Dictionary Error: " .. tostring(loadErr))
    count = 0
end
